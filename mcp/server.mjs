...
const server = new McpServer(
  ...
  {
    instructions:
      "Render and update the native Yogurt AI canvas. Inspect source-aware page or selection context with get_cowart_thinking_context, preview and atomically apply typed local edits with apply_cowart_thinking_operations, attach project materials with import_cowart_material, and use undo_cowart_thinking_operation for guarded undo. Route a mixed brief into a structured page layout plan first, then create one reviewable full-canvas layout blueprint that shows the planned image, editable-structure, and evidence slots. Never substitute a concept image, scene image, or poster for that layout blueprint. After approval, place each generated image, native editable structure, and evidence card into its approved slot; product semantics come from the original sources, never from blueprint pixels or OCR. Build canvas line diagrams natively with a semanticDiagram batch plus semantic zones, cards, and relations so every object stays editable and traceable. Reuse insert_cowart_image and insert_cowart_html_draft only for visual assets or an explicitly requested precision inline-SVG fallback instead of hand-writing tldraw records.",
  },
);
