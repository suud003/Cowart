...
const server = new McpServer(
  ...
  {
    instructions:
      "Render and update the native Yogurt AI canvas. Inspect source-aware page or selection context with get_cowart_thinking_context, preview and atomically apply ordinary additive edits with apply_cowart_safe_thinking_operations, attach project materials with import_cowart_material, and use undo_cowart_thinking_operation for guarded undo. Reserve apply_cowart_thinking_operations for explicitly authorized deletion or user-authored edits. Route a mixed brief into a structured v3 page plan with route-specific content specs, and validate it with validate_cowart_auto_compose_plan before image generation. Attempt one near-final full-page composition reference in which visual scenes, diagram topology, evidence cards, hierarchy, and whitespace already resemble the intended page; never substitute empty placeholder boxes, an abstract wireframe, or one concept image/poster. Image-preview generation has an initial attempt plus one retry. Authentication, access, rate-limit, network, tool, or service failure degrades only visual slots: in autonomous mode continue native diagram and evidence slots without confirmation, and never mark the whole task failed. In guided mode pause once after a verified preview or a fallback plan-only checkpoint. Final product semantics come from the page plan and original sources, never generated pixels or OCR. Build canvas diagrams natively with semanticDiagram layoutEngine=html-line-svg and balanced fixed-slot fitting: create one keyed semantic zone with explicit validated slot bounds, then create all cards and relations in the same batch; parent every card to the zone and omit manual card coordinates or anchors. Require layoutApplied=true and a valid dry-run layoutReport before apply. Use insert_cowart_image for managed visual assets and insert_cowart_html_draft only for an explicitly requested precision inline-SVG fallback instead of hand-writing tldraw records.",
  },
);
