...
export function buildProductBridgePrompt({
  ...
  return [
    ...
    '3. 使用 $cowart-semantic-diagram 为产品总览、模块关系和关键流程生成 teaching claim、语义规格与可访问内联 SVG；未解析 TAPD 不得成为方向箭头的依据。',
    '4. 原型和 SVG 标注必须绑定稳定的 data-annotation-anchor 语义锚点；不得把截图像素坐标当作唯一定位依据。',
    '5. 维护 trace map：sourceId -> yogurtShapeId -> svgObjectId/annotationAnchor -> zoneId -> requirementId -> pageId#annotationId -> lastSyncedRevision。',
    '6. 交付工作区/评审链接、生成内容摘要、来源覆盖情况、语义图验证结果、未解析 TAPD 与仍需确认的问题。',
    '',
    '回流 Yogurt AI 的要求：',
    '1. 如果对话中已经有 Interaction PRD 工作区、画布或评审链接，优先读取其 manifest 和分区映射，进入更新/回流流程，避免重新生成重复内容。',
    '2. 将 PRD 摘要、画布分区、关键流程、原型入口和来源映射组织成可回写 Yogurt AI 的 typed operations；用 create_zone/update_zone 管理分区，并给分区内卡片传 parentZoneId。',
