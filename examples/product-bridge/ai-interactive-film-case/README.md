...
“分岔回声”展示了 Yogurt AI 的完整产品体验：先把世界氛围、玩家循环、用户证据和研发约束组织进一张视觉项目画布，再由 Product Bridge 把这些材料整理成结构化 PRD、可直接操作的交互原型和统一评审空间，最后把确认后的产品结构带回画布继续思考。

![Yogurt AI 的分岔回声视觉项目画布，集中展示世界语气、玩家循环、用户证据和研发约束](docs/images/yogurt-ai-visual-workspace.png)

![分岔回声 Product Bridge 评审空间，展示互动播放原型、PRD 与稳定标注](docs/images/product-bridge-review.png)

## 从一个想法到一套产品方案

本案例以“做一款 AI 互动影游”为输入。故事设定、目标用户、指标和商业判断均标记为待确认的 AI 假设，方便体验完整流程，又不会把推测包装成真实需求。

| Product Bridge 帮你完成 | 本案例中的可见结果 |
| --- | --- |
| 建立视觉项目全景 | 世界语气、玩家循环、用户访谈、本地需求摘录和风险约束集中在一张画布 |
...
视觉项目画布把参考图、氛围色板、核心循环、机制备忘、用户原话、本地需求摘录和风险便签放在同一个空间。Codex 入口保持在右下角，需要时展开为覆盖式工作台；画布不会被挤压，用户可以一边看全局，一边让 Agent 整理选区或生成 PRD。
