...
你可以直接输入要求，也可以从 `整理选区`、`生成 PRD`、`生成框线图` 开始。执行过程中，工作台会持续显示回复、计划、修改摘要和任务状态；当 Agent 需要范围、选项或参数时，结构化表单会直接出现在动态流中。外部授权请求只展示目标域名，并在你明确确认后由桌面应用安全打开。你也可以批准或拒绝受控操作，或随时中断任务。再次打开同一个项目时，桌面应用会继续使用该项目已保存的 Agent 会话。
