...
Yogurt AI 是一个可安装的 AI 产品工作台，同时提供独立桌面应用和 Codex 插件。它把 tldraw 原生无限画布与 Codex Agent 放进同一个界面：你可以整理本地文档、笔记、图片、对话和直接粘贴的需求摘录，圈选一组真实画布对象，再让 Agent 梳理材料、补全产品结构、生成框线图、PRD 或交互原型。

Agent 会读取当前页面和稳定对象 ID，持续回传执行状态、计划与修改摘要；遇到敏感操作时，审批就在工作台内完成。框线图和知识结构会回到画布成为可选择、可拖拽、可改写的原生对象；PRD 与原型保存在可继续评审的项目工作区，并可在确认后把关键结论回流画布。
...
1. 从 [GitHub Releases 下载 Yogurt AI Beta 0.2.5](https://github.com/suud003/Cowart/releases/tag/v0.2.5%2Bcodex.20260827) 的 `Yogurt-AI-Beta-Setup-0.2.5-x64.exe`。
...
```mermaid
flowchart LR
  A["本地文档 / 笔记 / 图片 / 需求摘录"] --> B["在画布整理证据、假设与问题"]
...
你可以直接输入要求，也可以从 `整理选区`、`生成 PRD` 开始；需要框线图时，直接用自然语言让 Agent 在当前画布生成可编辑结构。执行过程中，工作台会完整保留回复、计划、修改摘要和任务状态；需要确认或补充信息时，阻塞卡片会置顶显示。应用在后台时，系统通知和任务栏提醒会提示新回复与待处理操作。外部授权请求只展示目标域名，并在你明确确认后由桌面应用安全打开。你也可以批准或拒绝受控操作，或随时中断任务。再次打开同一个项目时，桌面应用会继续使用该项目已保存的 Agent 会话。

<p align="center">
  <img src="docs/images/yogurt-ai-codex-agent-workbench.png" width="100%" alt="Codex Agent 以覆盖式工作台打开在 Yogurt AI 互动影游画布上">
</p>
<p align="center"><sub>画布保持在原位；Agent 的上下文、快捷任务、完整对话、审批和输入区集中在同一个覆盖层中。</sub></p>
