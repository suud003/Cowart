...
对于同时包含画面、流程和约束的需求，Agent 会先生成一张低文字的整体参考图并暂停等待确认；确认后，所有视觉部件都沿用这张参考图，流程与关系则根据原始需求重建为可选择、可拖拽、可改写的画布对象。PRD 与原型仍是按需调用的独立产品工作区，不会被混合编排自动触发。
...
1. 从 [GitHub Releases 下载 Yogurt AI Beta 0.2.6](https://github.com/suud003/Cowart/releases/tag/v0.2.6%2Bcodex.20260827) 的 `Yogurt-AI-Beta-Setup-0.2.6-x64.exe`。
...
| 同时包含场景、玩法流程和产品约束的一段需求 | 先路由并生成整体参考图；确认后按同一参考生成视觉部件与原生结构 | 视觉统一、语义可编辑、来源可追溯的一张产品画布 |
...
<p align="center"><sub>真实桌面渲染：画布保持在原位；“智能编排”入口说明参考图优先的混合需求工作流。</sub></p>
...
只要需求里包含视觉内容，第一阶段就只生成一张整体参考图：它负责统一画面气质、配色、光线与构图，但不承载需要准确阅读的文字或产品逻辑。参考图写回当前画布后，Agent 会停下来等待确认。第二阶段才会把同一张已确认图片作为真实图像参考，分别生成场景或角色素材；玩法循环、系统关系和产品约束仍从原始文字与来源生成，因此保持原生可编辑，也不会从参考图像素里猜产品事实。
...
- `把选区整理成一张说明核心判断的框线图。`
- `用智能编排处理这段互动影游需求：先出整体参考图，确认后再分别生成场景图、可编辑玩法循环和约束卡片。`
...
- `cowart-thinking-canvas:cowart-auto-compose`：先路由混合需求；存在视觉内容时先建立可确认的整体参考图，再生成一致的图片、原生结构和证据卡片。
