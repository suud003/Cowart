...
对于同时包含画面、流程和约束的需求，Agent 会先规划整张页面，再生成一张低文字的画布布局蓝图供你确认。它展示图片区、可编辑结构区和证据区的位置、比例、层级与阅读顺序；确认后，各部分按对应槽位生成并落位。蓝图不是概念图，也不承载产品事实；流程、关系和约束始终根据原始材料生成，保持可选择、可拖拽、可改写。PRD 与原型仍是按需调用的独立工作区，不会被混合编排自动触发。

<p align="center">
  <img src="docs/images/yogurt-ai-workspace.png" width="100%" alt="Yogurt AI 的视觉项目工作台，以世界语气、玩家循环和证据约束组织互动影游材料">
</p>
<p align="center"><sub>空画布会展示一个明确标记的互动影游示例；添加真实对象后，示例自动隐藏。Codex 在右下角持续汇报状态，需要时再展开。</sub></p>

## Windows 桌面应用（Beta）

普通用户不需要安装 Node.js、Git 或全局 Codex CLI。使用 Windows x64 安装包即可开始：

1. 从 [GitHub Releases 下载 Yogurt AI Beta 0.2.7](https://github.com/suud003/Cowart/releases/tag/v0.2.7%2Bcodex.20260828) 的 `Yogurt-AI-Beta-Setup-0.2.7-x64.exe`。
...
| 同时包含场景、玩法流程和产品约束的一段需求 | 先路由并生成整页布局蓝图；确认页面分区后按槽位生成视觉部件、原生结构与证据 | 版面完整、语义可编辑、来源可追溯的一张产品画布 |
...
你可以直接输入要求，也可以从 `智能编排`、`整理选区`、`生成 PRD` 开始；需要框线图时，直接用自然语言让 Agent 在当前画布生成可编辑结构。执行过程中，工作台会完整保留回复、计划、修改摘要和任务状态；需要确认或补充信息时，阻塞卡片会置顶显示。应用在后台时，系统通知和任务栏提醒会提示新回复与待处理操作。外部授权请求只展示目标域名，并在你明确确认后由桌面应用安全打开。你也可以批准或拒绝受控操作，或随时中断任务。再次打开同一个项目时，桌面应用会继续使用该项目已保存的 Agent 会话。

<p align="center">
  <img src="docs/images/yogurt-ai-auto-compose-workbench.png" width="100%" alt="Yogurt AI Agent 工作台中的智能编排入口">
</p>
<p align="center"><sub>真实桌面渲染：画布保持在原位；“智能编排”会先确认整页布局蓝图，再生成各分区内容。</sub></p>
...
第一阶段不是生成电影概念图，而是先建立结构化页面计划，再输出整张画布的布局蓝图。蓝图会预演每个图片区、原生框线图区和证据卡片区的边界、相对尺寸、层级、留白与阅读顺序。它写回当前画布后，Agent 会停下来等待你确认页面布局。

第二阶段才按已确认的槽位逐项生成：视觉内容使用对应区域的画幅与构图，玩法循环和系统关系生成原生可编辑对象，约束与来源保留为可追溯卡片。最终坐标来自结构化布局计划，而不是从参考图像素或 OCR 猜出来；普通场景概念图只能作为视觉素材，不能冒充页面布局蓝图。
...
- `把选区整理成一张说明核心判断的框线图。`
- `用智能编排处理这段互动影游需求：先出整页布局蓝图，确认分区后再按槽位生成场景图、可编辑玩法循环和约束卡片。`
...
- `cowart-thinking-canvas:cowart-auto-compose`：先路由混合需求并建立可确认的整页布局蓝图，再按批准槽位生成图片、原生结构和证据卡片。
