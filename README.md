...
对于同时包含画面、流程和约束的需求，Agent 会先生成一张接近成品的整页视觉预演：场景图、流程节点、关系连线、证据卡片、标题层级和留白都会在一张完整页面中大致成形。随后它按同一套分区分别生成视觉素材，并把流程与约束重建为原生可编辑对象。页面计划在生成前会检查越界、重叠、间距和内容容量，避免最终画布挤在一起。你可以选择预演后暂停，也可以开启“自动推进画布”，让可逆的画布步骤连续完成。PRD 与原型仍是按需调用的独立工作区，不会被混合编排自动触发。
...
1. 从 [GitHub Releases 下载 Yogurt AI Beta 0.2.8](https://github.com/suud003/Cowart/releases/tag/v0.2.8%2Bcodex.20260829) 的 `Yogurt-AI-Beta-Setup-0.2.8-x64.exe`。
...
<p align="center"><sub>真实桌面渲染：画布保持在原位；“自动推进画布”开启后，整页预演通过校验便会继续完成各分区。</sub></p>
...
第二阶段严格沿用同一组槽位：视觉内容把对应预演区域作为构图和风格参考；玩法循环和系统关系依据原始材料重建为原生可编辑对象；约束与来源重建为可追溯卡片。写入前还会检查真实节点、文字和连线边界，发现拥挤会先重新排布，而不是把重叠结果写进画布。预演图片只负责视觉和构图，不能替代产品语义。

默认使用“分步确认”，整页预演完成后暂停一次。开启“自动推进画布”后，这个产品流程检查点会被跳过，可逆的画布步骤在同一任务里连续完成；该偏好不会自动批准 Codex 的命令、文件修改、外部授权、权限提升或信息请求。
...
- “自动推进画布”只跳过智能编排的整页预演确认，不会自动回复审批、提交表单、打开外部网址或扩大 Codex 权限。
