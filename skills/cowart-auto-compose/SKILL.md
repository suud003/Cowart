...
8. Dry-run the complete native diagram/evidence batch and inspect returned bounds before applying. Require every result to stay inside its slot, every pair of unrelated objects to have a visible gap, every label to remain readable, and every relation to avoid unrelated nodes and text. Repack once if needed. If it still fails, revise and re-preview the affected plan once in autonomous mode, or request a plan revision in guided mode; never apply tangled or clipped geometry.
