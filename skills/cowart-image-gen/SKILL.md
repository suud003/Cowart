...
5. Inspect every generated region against its slot and content spec. If any slot is missing, placeholder-like, or has materially wrong topology/density, regenerate once. The initial call plus this retry are the full attempt budget. If the second attempt fails, stop without insertion and return a concise typed reason (`quality`, `authentication`, `access`, `rate-limit`, `network`, `tool-unavailable`, or `service`) to the auto-compose caller. Do not make a third prompt-only or reference-free attempt. The caller owns degraded continuation of native routes; this image failure is not a whole-page task failure.
