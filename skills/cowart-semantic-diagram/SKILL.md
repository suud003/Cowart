...
When `$cowart-auto-compose` delegates a block, stay in native-only mode: use editable cards, semantic zones, and bound relations even when the standalone skill would otherwise consider HTML + inline SVG. Honor the validated slot bounds, inset content rectangle, diagram `contentSpec`, and reading order. The full-page composition reference may guide broad geometry, density, palette, and rhythm, but every object, label, direction, state, and relation must come from the shared content spec and traceable sources, never pixels or OCR. Keep at most 8 nodes and 10 relations in one slot. Dry-run the complete block, verify that real node/label/edge bounds remain inside its content rectangle without unrelated crossings, and repack once before invalidating the plan; never overflow, tangle, or silently rasterize it.
