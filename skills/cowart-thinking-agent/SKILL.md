...
- Use `$cowart-auto-compose` when one request mixes visual scenes or assets with flows, relationships, evidence, or constraints and the user expects Yogurt AI to decide the representation. Let it create one near-final full-page visual preview, then rebuild images, native editable structure, and evidence from the same validated page-plan slots and content specs.
